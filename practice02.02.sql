...
select cinsiyet , round(avg(yas),2) from manav group by cinsiyet;
...
select urun_adi,max(urun_miktar),min(urun_miktar)
from manav 
group by urun_adi
order by max desc;
...
--group by komutundan sonra where degil having kullanilir.
--aggregate fonk ile where kullanilmaz having kullanilir.


-- SORU11: Satılan urun_adi'na göre gruplayarak MAX ürün sayılarını sıralayarak 
--listeleyen sorgu yazınız.
-- NOT: Sorgu sadece MAX urun_miktari MIN urun_miktari na eşit olmayan kayıtları listelemelidir


select urun_adi, max(urun_miktar) as max_miktar
from manav 
group by urun_adi
having max(urun_miktar)!=min(urun_miktar)
order by max(urun_miktar);
